...
import { getApprovedManuals, getUserDrafts } from "../data/manualdata";
...
const Feeds = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Load user's drafts
      setUserDrafts(getUserDrafts(parsedUser.id));
    }
  ...
  const sortedFeeds = useMemo(() => {
    // Combine user drafts with approved manuals
    let feeds = [...userDrafts, ...approvedManuals];
    if (tagFilter !== "All") {
  ...
  }, [selectedCategory, tagFilter, approvedManuals, userDrafts]);

  const handleCardClick = (manual) => {
    // If it's a draft, go to edit page instead
    if (manual.status === "draft") {
      navigate(`/edit-manual/${manual.id}`);
    } else {
        ...
        <div className="home-manuals-grid">
          {sortedFeeds.map((manual) => (
            <div
              ...
              style={{
                cursor: manual.status === "draft" ? "pointer" : "pointer",
              }}
            >
